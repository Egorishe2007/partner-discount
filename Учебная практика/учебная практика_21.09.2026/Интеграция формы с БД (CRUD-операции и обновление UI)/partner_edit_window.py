...
class PartnerEditWindow(tk.Toplevel):
    ...
    def prepared_values(self) -> dict:
        """Подготовить значения к записи: рейтинг уходит в базу числом."""
        values = self.values()
        values["rating"] = int(values["rating"] or 0)
        return values
    ...
    def save(self):
        """Записать карточку в базу и вернуться в обновленный реестр."""
        try:
            values = self.prepared_values()
        except ValueError:
            self.show_status("Рейтинг должен быть целым числом от 0.")
            return
        ...
        try:
            ...
            self.show_status(f"Не удалось сохранить: {first_line(error)}")
            return
